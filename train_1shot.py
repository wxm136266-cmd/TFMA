...
import ssl
from image_data_get import SAC_4way_image , create_set
...
parser.add_argument('--dataset', default='SAC', help='Dataset (Only CWRU or PDB)') #choices=['CWRU', 'PDB']
...
parser.add_argument('--episode_num_train', type=int, default=100, help='Number of training episodes') #130
parser.add_argument('--episode_num_test', type=int, default=150, help='Number of testing episodes')
parser.add_argument('--way_num_CWRU', type=int, default=4, help='Number of classes for CWRU')
...
parser.add_argument('--spectrum', default=True, help='Use spectrum') #action='store_true'
...
if args.dataset == 'SAC':
    ...
    train_dataset_SAC = FewshotDataset(train_x_SAC ,train_y_SAC, episode_num=args.episode_num_train, way_num=args.way_num_CWRU, shot_num=1, query_num=1)
    train_dataloader_SAC = DataLoader(train_dataset_SAC, batch_size=args.batch_size, shuffle=True)
    test_dataset_SAC = FewshotDataset(test_x_SAC, test_y_SAC, episode_num=args.episode_num_test,way_num=args.way_num_CWRU, shot_num=1, query_num=1)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with tqdm(train_dataloader, desc=f'Epoch {epoch}/{num_epochs}', unit='batch') as t:
            for query_images, query_targets, support_images, support_targets in t:
                ...
                s = support_images.permute(1, 0, 4, 2, 3).to(device)
        ...
        with torch.no_grad():
            ...
            acc,test_loss,precision_test, recall_test, f1_test = cal_accuracy_fewshot_ensemble_1shot(test_loader, net, device,loss1, loss2)
            writer.add_scalar('data/val_loss', float(test_loss), test_global_count)
            writer.add_scalar('data/test_acc', float(acc), test_global_count)
            ...
            if acc > pred_acc:
                ...
                pred_acc = acc
                model_name = f'{args.model_name}_1shot_{acc:.4f}_{training_samples}samples_3channel_1204_v2.pth'
                torch.save(net, path_weight + model_name)
                print(f'=> Save the best model with accuracy: {acc:.4f}')
        torch.cuda.empty_cache()
    model_end = f'{args.model_name}_1shot_{acc:.4f}_{training_samples}samples_end_3channel_A0327.pth'
...
writer = SummaryWriter('./checkpoints/3channels/accloss/1shot-30samples_A_0327')
...
if args.dataset == 'CWRU':
    ...
    print('end training...................!!')
writer.close()
# for query_images, query_targets, support_images, support_targets in train_dataloader_SAC:
